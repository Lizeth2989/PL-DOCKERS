...
sudo ip -n $container_name1 link add $veth_name_in_container1 type veth peer name $veth_name_in_container2 netns $container_name2
if [ $? -ne 0 ]
then
	clean_up
fi
...
function add_veth_to_container ()
{
  ...
  sudo ip netns exec $container_name ip link set $veth_name_in_container up
